...
const invoiceCommentController = require('../controllers/invoiceCommentController');
...
router.post('/:invoiceId/comments', writeRateLimit, requirePermission(['invoices.view'], { requireAll: false }), [
    body('comment').notEmpty().trim().withMessage('El comentario no puede estar vacío')
], invoiceCommentController.createComment);
